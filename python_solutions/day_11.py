...
class Monkey:
    ...
    def throw(self):
        while self._items:
            ...
            item_worry = self._operation(item_worry) // self._worry_divisor
...
def monkey_factory(spec: str, worry_divisor: int) -> Monkey:
    ...
    if len(mult_matches) == 1:
        operation = partial(mul, int(mult_matches[0].split()[1]))
    ...
    return Monkey(initial_items=[int(i) for i in items], operation=operation, worry_divisor=worry_divisor)


def target_monkey_function(if_divisible_by: int, then_monkey: Monkey, else_monkey: Monkey, worry_level: int) -> Monkey:
...
def target_monkey_function_factory(spec: str, monkeys: List[Monkey]) -> Callable[[int], 'Monkey']:
    
    if_divisible_by = extract_integer_after_keyphrase(spec, "Test: divisible by")
    then_monkey_index = extract_integer_after_keyphrase(spec, "If true: throw to monkey")
    else_monkey_index = extract_integer_after_keyphrase(spec, "If false: throw to monkey")

    return partial(target_monkey_function, if_divisible_by, monkeys[then_monkey_index], monkeys[else_monkey_index])
...
def load_monkeys(worry_divisor: int):
    ...
    for monkey_spec in monkey_specs:
        monkeys.append(monkey_factory(monkey_spec.strip(), worry_divisor))

    for monkey, spec in zip(monkeys, monkey_specs):
        monkey.set_target_monkey_function(target_monkey_function_factory(spec.strip(),  monkeys))
...
if __name__ == '__main__':
    ...
    print(f"Part 1 monkey business: {sorted(p1_num_inspections_per_monkey)[-2] * sorted(p1_num_inspections_per_monkey)[-1]}")
