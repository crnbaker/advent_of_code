	...
	def _update_tail(self) -> None:
		...
		if round(displacement.distance) > self._length:
			if (displacement.angle % 90) == 0:
				...
				self._tail.unit_move(Direction(round_angle_to_nearest_diagonal(displacement.angle)))
...
if __name__ == '__main__':
	...
	plot([p[0] for p in rope._head_log], [p[1] for p in rope._head_log], 'gx')
	plot([p[0] for p in rope._tail_log], [p[1] for p in rope._tail_log], 'r*')
	show()
